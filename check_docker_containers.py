...
def run_image(image_name):
    stream = subprocess.Popen('docker run -e EULA=TRUE ' + image_name, shell=True)


def anaylze_docker_container(image_name, file_saved):
    print("Anyalzing image {}".format(image_name))
    ...
    json_res['keyword_search'] = interesting_places
...
anyalze_containers(["serverninja/minecraft:v0.0.6", "couchbase/server-sandbox:6.6.0", "008ak89/ssh_enabled_server:1.0", "00theballs00/web_server", "openhack/minecraft-server"], "/home/student/Research/explore_docker/results")
print("done")
